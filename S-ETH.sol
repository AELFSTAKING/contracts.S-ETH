...
contract StandardToken is ERC20,Pausable,Blacklisted {
    ...
    event CurrentBalanceByHeight(address from, uint256 fromBalance, address to, uint256 toBalance);
    ...
    */
    function transfer(address _to, uint256 _value) whenNotPaused notInBlacklist(msg.sender) notInBlacklist(_to) public returns (bool) {
        ...
        emit CurrentBalanceByHeight(msg.sender, balances[msg.sender], _to,  balances[_to]);
        return true;
    }
    ...
    */
    function transferFrom(address _from, address _to, uint256 _value) whenNotPaused notInBlacklist(msg.sender) notInBlacklist(_from) notInBlacklist(_to) public returns (bool) {
        ...
        emit CurrentBalanceByHeight(_from, balances[_from], _to,  balances[_to]);
        return true;
    }
...
/**
 * @title StakingDex Token
 * @dev Token that can create Stacking Dex orders.
 */
contract StakingDexToken is BurnableToken{

    event TransferWithMsg(address indexed from, address indexed to, uint256 value, string mesage);

    /**
    * @dev create pending order
    * @param _to address The address which you want to send tokens to
    * @param _value uint256 The amount of token to be spended
    * @param _message message
    */
    function orderCreate(address _to, uint256 _value, string _message) public returns (bool) {
        bool result = transfer(_to, _value);
        require(result);
        StackingDex(_to).orderCreate(msg.sender, _value, _message);
        return true;
    }

    /**
    * @dev transfer with message
    * @param _to address The address which you want to send tokens to
    * @param _value uint256 The amount of token to be spended
    * @param _message message
    */
    function transferWithMsg(address _to, uint256 _value, string _message) public returns (bool) {
        bool result = transfer(_to, _value);
        require(result);
        emit TransferWithMsg(msg.sender, _to, _value, _message);
        return true;
    }
    ...
    */
    function transferToMultiAddresses(address[] _addresses, uint256[] _amounts, string _message) public returns (bool) {
        uint256 totalAmount = 0;
        for (uint256 i = 0; i < _addresses.length; i++) {
            bool result = transfer(_addresses[i], _amounts[i]);
            totalAmount = totalAmount + _amounts[i];
            require(result);
        }
        emit TransferWithMsg(msg.sender, address(0), totalAmount, _message);
        return true;
    }
}
...
*/
contract SETHToken is StakingDexToken {
    string public constant name = "S-ETH";
